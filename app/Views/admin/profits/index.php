    ...
    <div class="col-sm-6 col-xl-3">
        <div class="card stat-card shadow-sm h-100">
            <div class="card-body">
                ...
                <div class="small text-secondary"><?= (int) $totals['available_count'] ?> entries not yet withdrawn</div>
            </div>
        </div>
    </div>
    <div class="col-sm-6 col-xl-3">
...
<div class="card shadow-sm mb-3">
    <div class="card-body">
        <?= form_open('admin/profits', ['method' => 'get', 'class' => 'row g-2 align-items-end']) ?>
            <div class="col-md-3">
                ...
                <input type="text" class="form-control form-control-sm" id="customer" name="customer"
                       placeholder="Name or email" value="<?= esc($filters['customer']) ?>">
            </div>
            <div class="col-md-2">
                <label class="form-label small mb-1" for="package">Package</label>
                <input type="text" class="form-control form-control-sm" id="package" name="package"
                       value="<?= esc($filters['package']) ?>">
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-1 d-flex gap-1">
                ...
                <a href="<?= site_url('admin/profits') ?>" class="btn btn-sm btn-outline-secondary">Clear</a>
            </div>
        <?= form_close() ?>
    </div>
</div>
        ...
        <table class="table table-hover align-middle mb-0">
            ...
            <?php foreach ($profits as $profit): ?>
                <tr>
                    ...
                    <td>
                        <div><?= esc($profit['customer_name']) ?></div>
                        <div class="small text-secondary"><?= esc($profit['package_name']) ?></div>
                    </td>
                    <td class="small text-secondary">
                        #<?= (int) $profit['investment_id'] ?> ·
                        <?= esc(schedule_label($profit['withdrawal_type'])) ?>
                    </td>
            ...
            <?php endforeach; ?>
            <?php if ($profits === []): ?>
                <tr><td colspan="7" class="text-center text-secondary py-4">No profit entries match these filters.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
